...
class TileTypes:
    ...
    Impassable = set(
        [WALL, PANELS, CHAIR, DOOR_CLOSED, CRATE, HEALTH_STATION, REDALERT_STATION, RECHARGE_STATION]
    )
...
class GameView(ui.RootElement):
    def __init__(self):
        ...
        self.nightlight.SetVertices(Point(0, 0), globals.screen_abs - Point(0, 0), 0.01)
        self.timeofday = TimeOfDay(0.1)
        # self.mode = modes.LevelOne(self)
        self.StartMusic()
        self.fixed_light = actors.FixedLight(Point(11, 38), Point(26, 9))
        self.interact_box = ui.Box(
            parent=globals.screen_root, pos=Point(0.3, 0.0), tr=Point(0.5, 0.08), colour=(0.5, 0.5, 0.5, 0.7)
        )
